...
async function githubModelsChat(messages, { jsonMode = false } = {}) {
  ...
  if (!text) throw new Error('GitHub Models returned no text');
  return text;
}
...
{"headline":"...","paragraph":"..."}`,
  ...
  const text = await githubModelsChat(messages, { jsonMode: true });
  const parsed = JSON.parse(text);
  if (!parsed.headline || !parsed.paragraph) {
    throw new Error('GitHub Models reduce step missing headline or paragraph');
  }
  return parsed;
}
...
export async function summarize(transcript, videoTitle) {
  if (!config.githubModelsToken) {
    throw new Error('GITHUB_MODELS_TOKEN (or GITHUB_TOKEN) is not set');
  }

  const chunks = chunkTranscript(transcript, config.githubModelsChunkChars);
  console.log(`[summarize:github-models] ${chunks.length} chunk(s) to process`);

  const notes = [];
  for (let i = 0; i < chunks.length; i++) {
    notes.push(await summarizeChunk(chunks[i], i, chunks.length));
    if (i < chunks.length - 1) await sleep(config.githubModelsPaceMs);
  }

  if (notes.length > 1) await sleep(config.githubModelsPaceMs);
